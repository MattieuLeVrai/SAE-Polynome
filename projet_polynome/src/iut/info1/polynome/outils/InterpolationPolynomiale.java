...
public class InterpolationPolynomiale { 
     ...
     * @return Le polynôme d'interpolation unique sous forme 
     * d'une instance de Polynome
     * @throws IllegalArgumentException si les tableaux sont null,
     * vides, de tailles différentes, ou si deux abscisses sont identiques.
     */
    public Polynome interpolerLagrange(double[] x, double[] y) {
        if (x == null || y == null) {
            throw new IllegalArgumentException(
            		  "Les tableaux de points ne peuvent pas être null.");
        }
        if (x.length != y.length) {
            throw new IllegalArgumentException(
            		  "Les tableaux x et y doivent avoir la même taille.");
        }
        if (x.length == 0) {
            throw new IllegalArgumentException(
            		  "Il faut au moins un point" +
                      "pour effectuer une interpolation.");
        }
        ...
        for (int i = 0; i < nombreDePoints; i++) {
            for (int j = i + 1; j < nombreDePoints; j++) {
                if (Math.abs(x[i] - x[j]) < 1e-12) {
                    throw new IllegalArgumentException(
                    	"Les abscisses (x) doivent toutes être distinctes "
        ...
        for (int i = 0; i < nombreDePoints; i++) {
            ...
            for (int j = 0; j < nombreDePoints; j++) {
                if (i != j) {
                    ...
                    // En tableau de coefficients (croissants) :
                	// [-x_j, 1.0] car -x_j*X^0 + 1.0*X^1
